...
**Colour themes** (`app/globals.css` + `lib/theme.ts`, chosen in the settings, §6.3): both are dark and keep the Bitcoin orange as the accent — only the neutrals change, so no component ever branches on the theme. `ocean` (default) is a deep navy with warm cream text; `night` is orange on near-black. A theme is one set of CSS custom properties under `[data-theme="…"]`, and every component styles itself with the tokens built on them, so switching is one attribute on `<html>` (`components/ThemeEffect.tsx`, rendered in the root layout so the start screen and the legal pages are themed too). The choice lives in the portfolio file (`settings.theme`, optional — older files fall back to the default) and is mirrored to a device preference (`localStorage`), exactly like the language: the file wins when one is opened. Charts are the one exception that cannot use the tokens — SVG presentation attributes take no CSS variables — so `lib/theme.ts` repeats the palette as literals for Recharts, and a test asserts that table and stylesheet never drift apart.
...
`public/demo-portfolio.json` and `…​.en.json` are what "Testportfolio laden" opens. They are not a stub: the demo doubles as the worked example of every feature, so the ledger contains every constellation the app knows — all four wallet types, a wallet with several accounts, all five transaction types, a batched transfer (nine buys in one send), a chain across three wallets, one send arriving in two accounts, foreign-currency settlement valued from history, external receives and sends, BTC and fiat fees, disposals on both sides of the holding period in two tax years, a watchlist with three address types, UTXO labels, an import preset with a row filter, and a dashboard layout.
