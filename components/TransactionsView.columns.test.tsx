...
describe("TransactionsView: the table on a narrow screen", () => {
  // A phone gets the same table as a desktop, all of it. It was briefly cut
  // down to four columns there, on the theory that eleven of them do not fit —
  // but the columns are the user's choice (CLAUDE.md §3.5), and a table that
  // silently drops most of them answers a different question than the one the
  // column picker was asked. What gives instead is the width: the table scrolls
  // sideways inside its own card, so nothing is hidden and the page itself
  // still never scrolls.
  it("hides no column at any width", () => {
    render(<TransactionsView />);
    const table = document.querySelector("table")!;
    const dropped = [...table.querySelectorAll("th, td")]
      .map((cell) => cell.className)
      .filter((cls) => /\bhidden\b/.test(cls));
    expect(dropped).toEqual([]);
  });

  it("lets the table scroll rather than the page", () => {
    render(<TransactionsView />);
    const table = document.querySelector("table")!;
