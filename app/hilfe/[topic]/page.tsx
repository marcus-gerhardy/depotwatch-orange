...
export function generateStaticParams() {
  return HELP_CONTENT.de.map((topic) => ({ topic: topic.id }));
}
...
export async function generateMetadata({
  ...
  const entry = HELP_CONTENT.de.find((t) => t.id === topic);
  return {
    title: `${entry?.title ?? "Hilfe"} — Hilfe — DepotWatch Orange`,
    description: entry?.summary,
    alternates: {
      canonical: helpTopicPath(topic, "de"),
